...
case "$(/bin/basename $0)" in
	...
	### Update 
	/usr/bin/emerge -auDN --keep-going ${pkgs:-@world} $ops	# update ops
	res=$? 
	...
	update_mods="$(tac /var/log/emerge.log |
	    sed -ne '0,/Started emerge on/s/.*>>> emerge.*\/\(python\|perl\|xorg-server\)-[0-9].*/\1/p')"
	...
	[ $exit_status -eq 0 ] && /usr/bin/emerge --quiet --depclean $ops
	res=$?							# depclean ops
	#echo "	Exit status: $res"
